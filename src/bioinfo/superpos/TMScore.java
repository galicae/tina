...
public class TMScore {
	...
	static double d, d0, d0_fix;
	static int nseqA, nseqB;
	static int m_out = -1;
	static int m_fix = -1;
	static int ier = 0;
	static int n_ali;
	...
	static double rms, drms, armsd, rmsd_ali, rmsd;
	static String fname = "";
	static String pdb = "";
	static String outname = "";
	static double d0_search;
	static double d_output;
	static int[] L_ini = new int[nmax];
	static int[] iA = new int[nmax];
	static int[] iB = new int[nmax];
	static int[] i_ali = new int[nmax];
	static int[] nresA = new int[nmax];
	static int[] nresB = new int[nmax];
	static double[][] u = new double[4][4];
	static double[] t = new double[4];
	static double[] iq = new double[nmax];
	static double[] xa = new double[nmax];
	static double[] ya = new double[nmax];
	static double[] za = new double[nmax];
	static double[] xt = new double[nmax];
	static double[] yt = new double[nmax];
	static double[] zt = new double[nmax];
	...
	/**
	 * TM score calculation. A slight overwrite (and overload) of the original
	 * TMscore jar by the Zhang server. This version calculates a somehow
	 * pessimistic TM score, probably due to how doubles are treated by java
	 * 
	 * @param pdb1
	 *            the first protein structure (the native)
	 * @param pdb2
	 *            second protein structure (prediction, in the original TMscore)
	 * @param nLeng
	 *            the length by which to normalize the TM score
	 * @return a double[5][4] array. It contains following values: 
	 * result[0] = t.clone();
	 * result[1] = u[1].clone();
	 * result[2] = u[2].clone();
	 * result[3] = u[3].clone();
	 * result[4][0] = score_max (TM score);
	 * result[4][1] = score_GDT (GDT);
	 * result[4][2] = rmsd_ali  (RMSD);
	 * @throws Exception
	 */
	public static double[][] doStuff(PDBEntry pdb1, PDBEntry pdb2, int nLeng)
			throws Exception {
		normLength = nLeng;
		int i, j;
		int k = 0;
		...
		for (i_init = 1; i_init <= n_init; i_init++) { // 333
			...
			for (iL = 1; iL <= iL_max; iL++) { // 300 !on aligned residues,
				...
				rms = u3b(r_1, r_2, LL, 1, rms, u, t, ier); // !u rotate r_1 to
															// r_2
				...
				for (j = 1; j <= nseqA; j++) {
					xt[j] = t[1] + u[1][1] * xa[j] + u[1][2] * ya[j] + u[1][3]
							* za[j];
					yt[j] = t[2] + u[2][1] * xa[j] + u[2][2] * ya[j] + u[2][3]
							* za[j];
					zt[j] = t[3] + u[3][1] * xa[j] + u[3][2] * ya[j] + u[3][3]
							* za[j];
				}
				d = d0_search - 1;
				score_fun(xt, yt, zt, xb, yb, zb); // !init, get scores,
													// n_cut+i_ali(i) for
				// iteration
				if (score_max < score) {
					score_max = score;
				...
				outer_301: for (int it = 1; it <= n_it; it++) { // 301
					...
					rms = u3b(r_1, r_2, LL, 1, rms, u, t, ier); // !u rotate r_1
																// to r_2
					for (j = 1; j <= nseqA; j++) {
						xt[j] = t[1] + u[1][1] * xa[j] + u[1][2] * ya[j]
								+ u[1][3] * za[j];
						yt[j] = t[2] + u[2][1] * xa[j] + u[2][2] * ya[j]
								+ u[2][3] * za[j];
						zt[j] = t[3] + u[3][1] * xa[j] + u[3][2] * ya[j]
								+ u[3][3] * za[j];
					}
					score_fun(xt, yt, zt, xb, yb, zb); // !init, get scores,
														// n_cut+i_ali(i) for
		...
		// System.out.printf("Structure1: %s  Length= %d \n", "PDB2", nseqB);
		// System.out.println("Normalized by " + normLength);
		...
		double score_GDT = (n_GDT1_max + n_GDT2_max + n_GDT4_max + n_GDT8_max)
				/ (float) (4 * normLength);
		// System.out.printf(
		// "GDT-TS-score= %s (d<1)= %s  (d<2)= %s (d<4)= %s (d<8)= %s\n",
		// df.format(score_GDT), df.format(n_GDT1_max / (float) (nseqB)),
		// df.format(n_GDT2_max / (float) (nseqB)),
		// df.format(n_GDT4_max / (float) (nseqB)),
		// df.format(n_GDT8_max / (float) (nseqB)));

		double score_GDT_HA = (n_GDT05_max + n_GDT1_max + n_GDT2_max + n_GDT4_max)
				/ (float) (4 * normLength);
		// System.out
		// .printf("GDT-HA-score= %s (d<0.5)= %s  (d<1)= %s (d<2)= %s, (d<4)= %s\n\n",
		// df.format(score_GDT_HA),
		// df.format(n_GDT05_max / (float) (nseqB)),
		// df.format(n_GDT1_max / (float) (nseqB)),
		// df.format(n_GDT2_max / (float) (nseqB)),
		// df.format(n_GDT4_max / (float) (nseqB)));
		...
		// System.out
		// .println(" -------- rotation matrix to rotate Chain-1 to Chain-2 ------");
		// System.out.printf("%-6s %-10s %-10s %-10s %-10s\n", "i", "t(i)",
		// "u(i,1)", "u(i,2)", "u(i,3)");
		// for (i = 1; i <= 3; i++) {
		// System.out.printf("%-6d %-10s %-10s %-10s %-10s\n", i,
		// df.format(t[i]), df.format(u[i][1]), df.format(u[i][2]),
		// df.format(u[i][3]));
		// }
		double[][] result = new double[5][4];
		result[0] = t.clone();
		...
		score_fun(xt, yt, zt, xb, yb, zb); // !give i_ali(i), score_max=score
											// now
		LL = 0;
		for (i = 1; i <= n_cut; i++) {
		...
		if (m_out == 1) {
			...
			for (i = 1; i <= nseqA; i++) {
				temp = String.format(
						"ATOM  %5d  CA  %3s%6d    %8.3f%8.3f%8.3f\n", nresA[i],
						NameMap(String.valueOf(seq1A.charAt(i))), nresA[i],
						xt[i], yt[i], zt[i]);
				fw.append(temp);
			}
			fw.append("TER\n");
			...
			for (i = 1; i <= nseqB; i++) {
				temp = String.format(
						"ATOM  %5d  CA  %3s%6d    %8.3f%8.3f%8.3f\n",
						2000 + nresB[i],
						NameMap(String.valueOf(seq1B.charAt(i))), nresB[i],
						xb[i], yb[i], zb[i]);
				fw.append(temp);
			}
			fw.append("TER\n");
		...
		for (i = 1; i <= n_cut; i++) {
			...
			dis = Math.sqrt((xt[j] - xb[k]) * (xt[j] - xb[k]) + (yt[j] - yb[k])
					* (yt[j] - yb[k]) + (zt[j] - zb[k]) * (zt[j] - zb[k]));
			if (dis < d_output) {
				iq[j] = 1;
			}
		}
		...
		// d_output, n_cut, rmsd);
		// System.out.printf(
		// ": denotes the residue pairs of distance  %s Angstrom\n",
		// df.format(d_output));
	...
	public static String NameMap(String residule) {
	...
	// 2, calculate TMscore
	static void score_fun(double[] xt, double[] yt, double[] zt, double[] xb,
			double[] yb, double[] zb) {
		double d_tmp = d;
		...
		w_outer: while (true) {
			...
			for (int k = 1; k <= n_ali; k++) {
				...
				j = iB[k]; // ![1,nseqB]
				dis = Math.sqrt((xt[i] - xb[j]) * (xt[i] - xb[j])
						+ (yt[i] - yb[j]) * (yt[i] - yb[j]) + (zt[i] - zb[j])
						* (zt[i] - zb[j]));
				// *** for iteration:
				if (dis < d_tmp) {
					n_cut = n_cut + 1;
					i_ali[n_cut] = k; // ![1,n_ali], mark the residue-pairs in
										// dis<d
				}
				// *** for GDT-score:
				if (dis <= 8) {
				...
				// *** for MAXsub-score:
				if (dis < 3.5) {
					score_maxsub_sum = score_maxsub_sum + 1
							/ (1 + (dis / 3.5) * (dis / 3.5));
				}
				// *** for TM-score:
	...
	public static double u3b(double[][] x, double[][] y, int n, int mode,
		...
		for (j = 1; j <= 3; j++) {
			for (i = 1; i <= j; i++) {
				m = m + 1;
				rr[m] = r[1][i] * r[1][j] + r[2][i] * r[2][j] + r[3][i]
						* r[3][j];
			}
		}

		double spur = (rr[1] + rr[3] + rr[6]) / 3.0;
		double cof = (((((rr[3] * rr[6] - rr[5] * rr[5]) + rr[1] * rr[6]) - rr[4]
				* rr[4]) + rr[1] * rr[3]) - rr[2] * rr[2]) / 3.0;
		det = det * det;
		...
		if (spur > 0) {
			...
			if (h > 0) {
				...
				if (mode != 0) {// compute a
					...
					d = a[1][1] * a[1][3] + a[2][1] * a[2][3] + a[3][1]
							* a[3][3];
			...
			if (mode != 0 && a_failed != 1) {// a is computed correctly
				...
				// compute t
				for (i = 1; i <= 3; i++) {
					t[i] = ((yc[i] - u[i][1] * xc[1]) - u[i][2] * xc[2])
							- u[i][3] * xc[3];
				}
	...
	public static void release_memory() {
		L_ini = null;
		iA = null;
		iB = null;
		i_ali = null;
