...
public class LocalSequenceGotoh extends Gotoh{
	...
	public static void main(String[] args){
		char[] x = {'A','A','A','Y','L','G','A','A','A','Y','A','A','A','W','A'};
		char[] y = {'A','A','A','A','A','A','Y','A','W','A'};

		Sequence sx = new Sequence("x", x);
		Sequence sy = new Sequence("y", y);
		int[][] matrix = new int[26][26];
		for(int i = 0; i != matrix.length; i++){
			matrix[i][i] = 1;
		}
		
		GlobalSequenceGotoh g = new GlobalSequenceGotoh(-1, -1, matrix);
		SequenceAlignment a = g.align(sx, sy);
