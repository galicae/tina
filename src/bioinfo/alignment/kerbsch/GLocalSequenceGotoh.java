...
public class GLocalSequenceGotoh extends Gotoh {
	...
	public GLocalSequenceGotoh(double gapOpen, double gapExtend, int[][] substMatrix) {
		super(gapOpen, gapExtend);
		this.substMatrix = substMatrix;
	}

	public GLocalSequenceGotoh(double gapOpen, double gapExtend, double[][] substMatrix) {
		super(gapOpen, gapExtend);
	...
	public SequenceAlignment align(Alignable sequence1, Alignable sequence2) {
		...
		for (int i = 1; i < xsize; i++) {
			for (int j = 1; j < ysize; j++) {
				tempScore[i][j] = score(substMatrix, seq1[i - 1], seq2[j - 1]);
