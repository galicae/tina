...
public class TinyOracle implements Oracle {
	...
	public TinyOracle() {
		super();
		this.gotoh = new LocalSequenceGotoh(-10.0, -2.0,
	...
	public LinkedList<PartialAlignment> findSimiliarSegments(
		...
		Sequence templateSequence = new Sequence(problem.getThreading()
				.getStructure().getID(), template);
		Sequence targetSequence = new Sequence(problem.getThreading()
				.getSequence().getId(), target);
		results.add(new PartialAlignment(problem, align(templateSequence,
				targetSequence)));
		return results;
	}
	 ...
	 */
	protected SequenceAlignment align(Sequence templateSequence,
			Sequence targetSequence) {
		return (SequenceAlignment) gotoh
				.align(templateSequence, targetSequence);
	}
